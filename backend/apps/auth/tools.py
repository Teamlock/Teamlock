...
def create_access_token(
    user: User, password: str, x_teamlock_key: str | None, x_teamlock_app: str | None
) -> Login:
    ...
    encoded_jwt: str = jwt.encode(
        token_data.dict(), settings.SECRET_KEY, algorithm=ALGORITHM
    )

    session_key: str = CryptoUtils.generate_sim()
    encrypted_password: str = user.encrypt_password_for_current_session(
    ...
    RedisTools.store(
        encoded_jwt, json.dumps(tmp_user), expire=access_token_expires.seconds
    )
    return Login(access_token=encoded_jwt, expireAt=expire.isoformat())
...
def is_admin(user: UserSchema = Depends(get_current_user)) -> bool:
    if not user.is_admin:
        raise HTTPException(
            detail="Not Allowed", status_code=status.HTTP_401_UNAUTHORIZED
        )
